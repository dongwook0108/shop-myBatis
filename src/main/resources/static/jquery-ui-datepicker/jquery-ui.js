   ...
   allowing multiple different settings on the same page. */
    ...
    function Datepicker() {
        ...
        this.regional[""] = { // Default regional settings
            ...
            monthNames: ["January", "February", "March", "April", "May", "June",
                ...
                "December"], // Names of months for drop-down and formatting
	 ...
	 */
        ...
        _connectDatepicker: function (target, inst) {
            ...
            input.addClass(this.markerClassName).on("keydown",
                    this._doKeyDown).on("keypress", this._doKeyPress).on(
                    "keyup", this._doKeyUp);
            this._autoSize(inst);
        ...
        _autoSize: function (inst) {
            if (this._get(inst, "autoSize") && !inst.inline) {
                ...
                if (dateFormat.match(/[DM]/)) {
                    ...
                    date.setMonth(
                            findMax(this._get(inst, (dateFormat.match(/MM/) ?
                                    "monthNames" : "monthNamesShort"))));
                    date.setDate(
                            findMax(this._get(inst, (dateFormat.match(/DD/) ?
	 ...
	 */
        _destroyDatepicker: function (target) {
            ...
            if (nodeName === "input") {
                inst.append.remove();
                inst.trigger.remove();
                $target.removeClass(this.markerClassName).off("focus",
                        this._showDatepicker).off("keydown",
                        this._doKeyDown).off("keypress", this._doKeyPress).off(
                        "keyup", this._doKeyUp);
	 ...
	 */
            ...
            if (inst.yearshtml) {
                ...
                setTimeout(function () {
                    ...
                    if (origyearshtml === inst.yearshtml && inst.yearshtml) {
                        inst.dpDiv.find(
                                "select.ui-datepicker-year:first").replaceWith(
                                inst.yearshtml);
                    }
        ...
        _checkOffset: function (inst, offset, isFixed) {
            var dpWidth = inst.dpDiv.outerWidth(),
                    ...
                    viewWidth = document.documentElement.clientWidth + (isFixed
            ...
            offset.top -= Math.min(offset.top,
                    (offset.top + dpHeight > viewHeight && viewHeight
                            > dpHeight) ?
	 ...
	 */
        ...
        _tidyDialog: function (inst) {
            inst.dpDiv.removeClass(this._dialogClass).off(
	 ...
	 */
        parseDate: function (format, value, settings) {
            ...
            var iFormat, dim, extra,
                    iValue = 0,
                    shortYearCutoffTemp = (settings ? settings.shortYearCutoff
                            : null) || this._defaults.shortYearCutoff,
                    shortYearCutoff = (typeof shortYearCutoffTemp !== "string"
                            ? shortYearCutoffTemp :
                            new Date().getFullYear() % 100 + parseInt(
        ...
        _ticksTo1970: (((1970 - 1) * 365 + Math.floor(1970 / 4) - Math.floor(
                        1970 / 100) +
                    ...
                    newDate = (date == null || date === "" ? defaultDate
                            : (typeof date === "string" ? offsetString(date) :
                                    (typeof date === "number" ? (isNaN(date)
                                                    ? defaultDate : offsetNumeric(date))
	 ...
	 */
        _attachHandlers: function (inst) {
            ...
            inst.dpDiv.find("[data-handler]").map(function () {
                ...
                $(this).on(this.getAttribute("data-event"),
        ...
        _generateHTML: function (inst) {
            ...
            if (maxDate) {
                ...
                while (this._daylightSavingAdjust(
                        new Date(drawYear, drawMonth, 1)) > maxDraw) {
                    drawMonth--;
                    if (drawMonth < 0) {
                        drawMonth = 11;
                        drawYear--;
            ...
            for (row = 0; row < numMonths[0]; row++) {
                ...
                for (col = 0; col < numMonths[1]; col++) {
                    ...
                    for (dow = 0; dow < 7; dow++) { // days of the week
                        day = (dow + firstDay) % 7;
                        thead += "<th scope='col'" + ((dow + firstDay + 6) % 7
                    ...
                    numRows = (isMultiMonth ? this.maxRows > curRows
                            ? this.maxRows : curRows : curRows); //If multiple months, use the higher number of rows (see #7043)
        ...
        _adjustInstDate: function (inst, offset, period) {
            var year = inst.selectedYear + (period === "Y" ? offset : 0),
                    ...
                    day = Math.min(inst.selectedDay,
                            this._getDaysInMonth(year, month)) + (period === "D"
                            ? offset : 0),
